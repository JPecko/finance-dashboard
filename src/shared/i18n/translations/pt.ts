import type { en } from './en'

export const pt: typeof en = {
  categories: {
    housing:       'Habitação',
    food:          'Restauração & Bar',
    groceries:     'Supermercado',
    transport:     'Transporte / Automóvel',
    health:        'Saúde',
    entertainment: 'Entretenimento',
    shopping:      'Shopping',
    utilities:     'Serviços',
    education:     'Educação',
    travel:        'Viagem',
    investing:     'Investimento',
    sport:         'Desporto',
    wellness:      'Wellness',
    roundup:       'Arredondamento',
    cashback:      'Cashback',
    salary:        'Salário',
    freelance:     'Freelance',
    investment:    'Lucro',
    reimbursement: 'Reembolso',
    interest:      'Juros',
    'invest-move': 'Mov. de Investimento',
    capital:       'Mov. de Capital',
    transfer:      'Transferência',
    revaluation:   'Atualização Mercado',
    other:         'Outro',
  },

  sharedExpenses: {
    title:          'Splitwise',
    typeLabel:      'Splitwise',
    payerMe:        'Eu paguei',
    payerOther:     'Outra pessoa pagou',
    payerLabel:     'Pago por',
    owesYou:        'Quem te deve?',
    totalAmount:    'Total da despesa',
    myShare:        'A minha quota',
    statusOpen:     'Pendente',
    statusSettled:  'Liquidado',
    markSettled:    'Marcar como liquidado',
    markOpen:       'Reabrir',
    filterLabel:    'Splitwise',
    filterBank:     'Banco',
    filterAll:      'Todos',
    pending:        'Pendente: {{amount}}',
    splitPending:   'Split · {{amount}} pendente',
    splitSettled:   'Split · {{amount}} liquidado',
    settleUp:       'Acertar contas',
    settleUpToggle: 'Marcar como acerto Splitwise',
    settleUpDesc:   'Marca todas as entradas Splitwise abertas como liquidadas ao guardar',
  },

  nav: {
    dashboard:    'Dashboard',
    accounts:     'Contas',
    transactions: 'Movimentos',
    recurring:    'Recorrências',
    groups:       'Grupos',
    investments:  'Investimentos',
    settings:     'Definições',
  },

  common: {
    save:       'Guardar',
    cancel:     'Cancelar',
    confirm:    'Confirmar',
    delete:     'Eliminar',
    edit:       'Editar',
    add:        'Adicionar',
    close:      'Fechar',
    export:     'Exportar',
    import:     'Importar',
    search:     'Pesquisar',
    loading:    'A carregar...',
    noData:     'Sem dados.',
    optional:   'Opcional',
    signOut:    'Sair',
    comingSoon: 'Em breve',
    open:       'Abrir',
    notes:      'Notas (opcional)',
  },

  sidebar: {
    themeLight:   'Modo claro',
    themeDark:    'Modo escuro',
    language:     'Idioma',
    settings:     'Definições',
    signOut:      'Sair',
    hideBalances: 'Ocultar saldos',
    showBalances: 'Mostrar saldos',
  },

  groups: {
    title:            'Grupos',
    subtitle:         'Divide despesas com amigos e família',
    addGroup:         'Novo Grupo',
    noGroups:         'Sem grupos',
    noGroupsDesc:     'Cria um grupo para começar a dividir despesas com amigos.',
    addFirst:         'Criar o primeiro grupo',
    loading:          'A carregar grupos...',
    members:          'Membros',
    addMember:        'Adicionar Membro',
    removeMember:     'Remover Membro',
    memberName:       'Nome',
    memberEmail:      'Email (opcional)',
    memberNamePlaceholder: 'Nome completo',
    entries:          'Despesas',
    addEntry:         'Adicionar Despesa',
    noEntries:        'Sem despesas',
    noEntriesDesc:    'Adiciona uma despesa para começar a controlar os custos partilhados.',
    balances:         'Saldos',
    debts:            'Quem paga a quem',
    noDebts:          'Tudo liquidado!',
    paidBy:           'Pago por',
    splitAmong:       'Dividido entre',
    totalAmount:      'Valor Total',
    yourShare:        'A tua parte',
    youAreOwed:       'Devem-te',
    youOwe:           'Deves',
    settled:          'Tudo liquidado',
    owes:             '{{from}} → {{to}}',
    groupName:        'Nome do Grupo',
    groupNamePlaceholder: 'ex: Viagem Portugal, Casa',
    currency:         'Moeda',
    editGroup:        'Editar Grupo',
    deleteGroup:      'Eliminar Grupo',
    deleteGroupConfirm: 'Eliminar este grupo e todas as suas despesas? Esta ação não pode ser desfeita.',
    editEntry:        'Editar Despesa',
    deleteEntry:      'Eliminar Despesa',
    deleteEntryConfirm: 'Eliminar esta despesa?',
    newEntry:         'Nova Despesa',
    splitEvenly:      'Dividir igualmente',
    splitByPercent:   '%',
    splitCustom:      'Divisão personalizada',
    splitSumMismatch: 'A soma das partes deve ser igual ao total',
    youInGroup:       'Tu',
    back:             'Voltar aos Grupos',
    netBalance:       'O teu saldo líquido',
    latestExpenses:   'Últimas despesas',
    widgetTitle:      'Grupos',
    widgetEmpty:      'Sem grupos ativos.',
    selectGroup:      'Selecionar grupo',
    group:            'Grupo',
    iPaid:            'Eu paguei',
    memberPaid:       'Membro pagou',
    notMember:        'Não és membro deste grupo',
    othersOweYou:     'Os outros devem-te',
    goToGroups:       'Ir para Grupos',
    noGroupsTabMsg:   'Ainda sem grupos. Cria um primeiro.',
    debitAccount:     'Conta a debitar',
    noLinkedAccount:  'Sem transação bancária',
    reimbursableNote: 'Valor total debitado · os outros devem-te {{amount}}',
    inGroup:          'Grupo: {{name}}',
    settleUp:         'Acertar Contas',
    settleUpConfirm:  'Isto irá criar entradas de liquidação para todas as dívidas atuais, repondo todos os saldos a zero.',
    settleUpEntry:    'Liquidação',
    settleUpDone:     'Tudo acertado!',
    settleUpPayFrom:  'Pagar da conta',
    settleUpReceiveTo: 'Receber na conta',
  },

  investments: {
    title:                'Investimentos',
    subtitle:             'Acompanha a tua carteira e ativos',
    noHoldings:           'Sem posições',
    noHoldingsDesc:       'Adiciona uma posição para começar a acompanhar esta carteira.',
    noInvestmentAccounts: 'Sem contas de investimento',
    noInvestmentAccountsDesc: 'Adiciona uma conta de investimento para começar a acompanhar a tua carteira.',
    addHolding:           'Adicionar Posição',
    editHolding:          'Editar Posição',
    holding:              'Posição',
    asset:                'Ativo',
    assetName:            'Nome do Ativo',
    assets:               'Ativos',
    addAsset:             'Adicionar Ativo',
    editAsset:            'Editar Ativo',
    noAssets:             'Sem ativos — adiciona um acima para começar a acompanhar preços.',
    deleteAssetConfirm:   'Eliminar este ativo? As posições associadas também serão removidas.',
    noAssetsToLink:       'Ainda não há ativos configurados.',
    addAssetsFirst:       'Adiciona ativos acima primeiro.',
    ticker:               'Ticker',
    isin:                 'ISIN',
    label:                'Label',
    quantity:             'Quantidade',
    avgCost:              'Custo Médio',
    currentPrice:         'Preço Atual',
    marketValue:          'Valor de Mercado',
    costBasis:            'Custo Base',
    pnl:                  'Ganho/Perda',
    investedBase:         'Capital Investido',
    totalValue:           'Valor Total',
    deleteConfirm:        'Eliminar esta posição?',
    units:                'Unidades',
    noLink:               'Sem ligação',
    selectHolding:        'Selecionar posição',
    investmentDetails:    'Detalhes do investimento',
    viewHoldings:         'Ver Posições',
    perUnit:              'por unidade',
    priceDate:            'Data do preço',
    purchaseDate:         'Data de compra',
    forecastTitle:        'Previsão de Crescimento',
    forecastSubtitle:     'Projeção a longo prazo com base em juro composto',
    forecastReturn:       'Retorno Anual',
    forecastInflation:    'Inflação',
    forecastContribution: 'Contribuição Mensal',
    forecastHorizon:      'Horizonte',
    forecastYears:        'a',
    forecastNominal:      'Nominal',
    forecastReal:         'Real (€ de hoje)',
    forecastInvested:     'Total Investido',
    incomeContribTitle:      'Investido via Rendimento',
    incomeContribSubtitle:   'Contribuições de rendimentos marcados como movimento de investimento, relacionadas com as compras que financiaram e avaliadas com o histórico real de preço',
    incomeContribTotal:      'Total Contribuído',
    incomeContribUnmatchedOnly: 'Não foi possível relacionar nenhuma destas contribuições com uma compra importada (mesma data/valor) — sem valorização disponível.',
    incomeContribUnmatchedNote: '{{count}} contribuição(ões) não relacionada(s) com uma compra importada — excluída(s) da valorização acima.',
    incomeContribNearest:    'compra mais próxima: {{date}} no total de {{amount}} ({{days}}d de distância)',
    incomeContribNoPurchases: 'esta conta não tem nenhuma compra importada',
    syncPrices:           'Sincronizar Preços',
    syncing:              'A sincronizar...',
    syncDone:             'Preços atualizados.',
    syncNoKey:            'Adiciona a tua API key do Twelve Data em Definições → Integrações.',
    syncNoTickers:        'Nenhum ativo com ticker para sincronizar.',

    simulatorTitle:             'Simulador de Investimento',
    simulatorSubtitle:          'Projeta o crescimento do portefólio com base nas tuas hipóteses',
    simulatorInitial:           'Montante Inicial',
    simulatorHorizon:           'Horizonte',
    simulatorTicker:            'Ativo / Ticker',
    simulatorTickerPlaceholder: 'ex: VUAA.DE, SXR8.DE, AAPL',
    simulatorGain:              'Ganho',
  },

  update: {
    title:     'Nova versão disponível',
    message:   'Há uma nova versão do Financelli disponível. Atualiza agora para teres as últimas melhorias.',
    updateBtn: 'Atualizar agora',
  },

  auth: {
    signIn:            'Iniciar Sessão',
    signUp:            'Criar Conta',
    signInTitle:       'Iniciar sessão',
    signUpTitle:       'Criar conta',
    appDescription:    'Gestão de finanças pessoais',
    signInDescription: 'Insere as tuas credenciais para aceder ao painel',
    signUpDescription: 'Cria uma nova conta para começar',
    name:              'Nome',
    email:             'Email',
    password:          'Palavra-passe',
    confirmPassword:   'Confirmar palavra-passe',
    namePlaceholder:   'O teu nome',
    emailPlaceholder:  'tu@exemplo.com',
    nameRequired:      'O nome é obrigatório',
    emailRequired:     'O email é obrigatório',
    passwordRequired:  'A palavra-passe é obrigatória',
    passwordMinLength: 'Mínimo de 6 caracteres',
    confirmRequired:   'Por favor confirma a palavra-passe',
    passwordMismatch:  'As palavras-passe não coincidem',
    switchToSignUp:    'Não tens conta?',
    switchToSignIn:    'Já tens conta?',
    signUpLink:        'Regista-te',
    signInLink:        'Inicia sessão',
    accountCreated:    'Conta criada! Verifica o teu email para confirmar e depois inicia sessão.',
    authFailed:        'Autenticação falhou.',
  },

  settings: {
    title:    'Definições',
    subtitle: 'Gere os teus dados, cópias de segurança e preferências',

    preferences:     'Preferências',
    preferencesDesc: 'Aparência e idioma.',
    theme:           'Tema',
    themeDesc:       'Escolhe o modo claro ou escuro',
    themeLight:      'Claro',
    themeDark:       'Escuro',
    language:        'Idioma',
    languageDesc:    'Em breve',

    profile:                'Perfil',
    profileDesc:            'O teu nome é mostrado na barra lateral.',
    displayName:            'Nome de exibição',
    displayNamePlaceholder: 'O teu nome',
    changePassword:         'Alterar Palavra-passe',
    newPassword:            'Nova palavra-passe',
    confirmPassword:        'Confirmar palavra-passe',
    changePasswordBtn:      'Alterar Palavra-passe',

    backup:          'Cópia de Segurança',
    backupDesc:      'Exporta uma cópia completa dos teus dados ou restaura uma anterior.',
    exportJson:      'Exportar Cópia JSON',
    exportJsonDesc:  'Todas as contas, movimentos e regras',
    importJson:      'Importar Cópia JSON',
    importJsonDesc:  'Isto irá substituir todos os dados existentes',
    csvExport:       'Exportar CSV',
    csvExportDesc:   'Exporta os teus movimentos como CSV para abrir no Excel ou Google Sheets.',
    exportCsv:       'Exportar Movimentos (CSV)',
    exportCsvDesc:   'Compatível com Excel e Google Sheets',
    exportCsvBtn:    'Exportar CSV',

    account:     'Conta',
    accountDesc: 'Sair da tua conta.',
    signOut:     'Sair',
    signOutDesc: 'Serás redirecionado para a página de login',

    maintenance:              'Manutenção',
    maintenanceDesc:          'Ferramentas de correção de dados (uso único).',
    recalcRoundups:           'Recalcular Roundups',
    recalcRoundupsDesc:       'Remove todos os roundups existentes e recria-os com base nos valores atuais das despesas. Executa uma vez para corrigir roundups que não foram atualizados após edição de despesas.',
    recalcRoundupsBtn:        'Recalcular',
    recalcRoundupsConfirm:    'Isto irá eliminar e recriar todos os roundups com base nos valores atuais das despesas.',
    recalcRoundupsDone:       'Concluído. Removidos {{removed}} roundups, recriados {{created}}.',

    integrations:              'Integrações',
    integrationsDesc:          'Liga serviços externos para melhorar a tua experiência.',
    priceApiKey:               'API Key do Twelve Data',
    priceApiKeyDesc:           'Usada para sincronizar preços dos ativos automaticamente. Obtém uma chave gratuita em twelvedata.com.',
    priceApiKeyPlaceholder:    'Cola a tua API key aqui',
    priceApiKeySaved:          'API key guardada.',

    dangerZone:     'Zona de Perigo',
    dangerZoneDesc: 'Estas ações são permanentes e não podem ser desfeitas.',
    clearData:      'Limpar Todos os Dados',
    clearDataDesc:  'Eliminar todas as contas, movimentos e regras',
    clearDataBtn:   'Limpar Dados',

    nameUpdated:      'Nome atualizado com sucesso.',
    passwordChanged:  'Palavra-passe alterada com sucesso.',
    dataImported:     'Dados importados com sucesso.',
    dataCleared:      'Todos os dados eliminados.',
    exportedJson:     'Cópia JSON exportada com sucesso.',
    exportedCsv:      'CSV exportado com sucesso.',
    passwordTooShort: 'São necessários pelo menos 6 caracteres',
    passwordMismatch: 'As palavras-passe não coincidem',
    importInvalid:    'Formato de ficheiro inválido.',
  },

  dashboard: {
    netWorth:           'Património',
    monthSummary:       'Resumo do Mês',
    accountBalances:    'Saldos das Contas',
    incomeVsExpenses:   'Receitas vs Despesas',
    incomeVsOutcome:    'Receitas vs Saídas',
    spendingByCategory: 'Despesas por Categoria',
    topExpenses:        'Maiores Despesas',
    perks:              'Benefícios e Vantagens',
    income:             'Receitas',
    expenses:           'Despesas',
    balance:            'Saldo',
    myIncome:           'As minhas Receitas',
    myExpenses:         'As minhas Despesas',
    myBalance:          'O meu Saldo',
    savingsRate:        'Taxa de Poupança',
    noAccounts:         'Sem contas.',
    noExpenses:         'Sem despesas este mês.',
    noPerks:            'Sem contas com cashback ou arredondamento.',
    total:              'Total',
    loading:            'A carregar os teus dados...',
    cashback:           'Cashback',
    roundup:            'Arredondamento',
    interest:           'Juros',
    investing:          'Investimento',
    ytd:                'Ano',
    savedPct:           'Poupaste {{rate}}% do rendimento',
    overspentPct:       'Gastos {{rate}}% acima do rendimento',
    investmentHistory:  'Histórico de Investimento',
    invested:           'Investido',
    portfolioValue:     'Valor da carteira',
    exportReport:       'Exportar PDF',
    report: {
      title:               'Relatório Mensal',
      subtitleCurrent:     '{{month}} · valores atuais (hoje)',
      subtitlePast:        '{{month}}',
      netWorthByType:      'Patrimônio Líquido por Tipo',
      excludesInvestments: 'Exclui contas de investimento — mostradas em baixo, já que não é possível reconstruir o valor de um mês passado.',
      investmentsSnapshot: 'Investimentos (valores atuais)',
      investmentsNotePast: 'Reflete os dados de mercado de hoje, não de {{month}} — não há histórico de posições guardado.',
      investmentsNoteNow:  'Reflete os dados de mercado atuais.',
      vs:                  'vs',
      pnl:                 'P&L',
      marketValue:         'Valor de Mercado',
      page:                'página',
    },
  },

  accounts: {
    title:          'Contas',
    totalBalance:   'Saldo total',
    addAccount:     'Adicionar Conta',
    noAccounts:     'Sem contas',
    noAccountsDesc: 'Adiciona as tuas contas bancárias, poupanças ou carteiras para começar a controlar as tuas finanças.',
    addFirst:       'Adicionar a primeira conta',
    sort:           'Ordenar',
    sortBy:         'Ordenar por',
    sortCreation:   'Ordem de criação',
    sortName:       'Nome A→Z',
    sortType:       'Tipo',
    sortColor:      'Cor',
    sortBalance:    'Saldo ↓',
    sortManual:     'Manual',
    colorOrder:     'Ordem das cores',
    dragToReorder:  'Arrasta os cartões para reordenar',
    saveOrder:      'Guardar ordem',
    share:          'Partilhar',
    edit:           'Editar',
    delete:         'Eliminar',
    updateValue:    'Atualizar Valor de Mercado',
    noBank:         'Sem banco',
    owner:          'proprietário',
    loading:        'A carregar contas...',
    deleteConfirm:  'Eliminar esta conta? Todos os movimentos associados serão mantidos.',
    sections: {
      current:    'Contas Correntes',
      savings:    'Poupanças',
      investment: 'Investimentos',
    },
    types: {
      checking:   'Conta à Ordem',
      savings:    'Poupança',
      investment: 'Investimento',
      cash:       'Dinheiro',
      credit:     'Cartão de Crédito',
      meal:       'Cartão Alimentação',
    },
    form: {
      newAccount:       'Nova Conta',
      editAccount:      'Editar Conta',
      name:             'Nome',
      namePlaceholder:  'ex: Despesas do Dia a Dia',
      nameRequired:     'O nome é obrigatório',
      bank:             'Banco',
      noBank:           'Sem banco',
      type:             'Tipo',
      balance:          'Saldo',
      initialBalance:   'Saldo Inicial',
      currency:         'Moeda',
      color:            'Cor',
      broker:           'Corretora',
      investedBase:     'Capital Investido',
      investedBaseDesc: 'Total depositado na corretora',
      entryFee:         'Fee por Entrada',
      entryFeeDesc:     'Comissão fixa por compra',
      cashback:         'Cashback %',
      cashbackDisabled: 'Desativado',
      roundup:          'Arredondamento',
      roundupTo:        'Destino do arredondamento',
      roundupToDesc:    'Escolhe uma conta para transferir o arredondamento, ou deixa como externo (é descontado da conta de origem, sem ser registado em lado nenhum)',
      roundupToExternal:'Externo (não registado)',
      saveChanges:      'Guardar Alterações',
    },
  },

  transactions: {
    title:              'Movimentos',
    addTransaction:     'Adicionar',
    noTransactions:     'Sem movimentos este mês',
    noTransactionsDesc: 'Começa a adicionar as tuas receitas e despesas para controlar os gastos mensais.',
    addFirst:           'Adicionar o primeiro movimento',
    income:             'Entradas',
    expenses:           'Despesas',
    outcome:            'Saídas',
    balance:            'Saldo',
    myExpenses:         'meu',
    searchPlaceholder:  'Pesquisar movimentos...',
    allAccounts:        'Todas as contas',
    allTypes:           'Todos os tipos',
    loading:            'A carregar movimentos...',
    colDate:            'Data',
    colDescription:     'Descrição',
    colAccount:         'Conta',
    colCategory:        'Categoria',
    colAmount:          'Valor',
    allCategories:      'Todas as categorias',
    filters:                  'Filtros',
    clearFilters:             'Limpar',
    applyFilters:             'Aplicar',
    filterAll:                'Todos',
    sharedWithParticipants:   'Partilhado com participantes',
    sharedWithParticipantsDesc: 'O valor é dividido entre todos os participantes da conta',
    splitBy:  'Dividir por',
    people:   'pessoas',
    myShare:  'minha parte',
    reimbursable:     'Reembolsável',
    reimbursableDesc: 'Pago em nome de outra pessoa — não contabilizado nas tuas despesas pessoais',
    roundup:          'Arredondamento',
    roundupDesc:      'Arredonda esta despesa e adiciona à poupança',
    personalFor:      'Despesa de',
    personalForDesc:  'Apenas as despesas pessoais deste participante incluirão este movimento',
    personalForMe:    'Só eu',
    personalForOther: 'Não contabilizado',
    newTitle:         'Novo Movimento',
    editTitle:        'Editar Movimento',
    category:         'Categoria',
  },

  recurring: {
    title:         'Recorrências',
    subtitle:      'Gere os teus rendimentos e despesas fixas',
    addRule:       'Adicionar Regra',
    editRule:      'Editar Regra',
    noRules:       'Sem regras recorrentes',
    noRulesDesc:   'Adiciona as tuas despesas fixas mensais como renda, subscrições, ou rendimentos regulares como o salário.',
    addFirst:      'Adicionar a primeira regra',
    nextDue:       'Próximo vencimento',
    total:         'Total',
    monthTotal:    'Total',
    byAccount:     'Balanço por Conta',
    frequency:     'Frequência',
    active:        'Ativo',
    inactive:      'Inativo',
    paused:        'Pausado',
    applyNow:      'Aplicar Agora',
    pause:         'Pausar',
    resume:        'Retomar',
    loading:                  'A carregar regras recorrentes...',
    deleteConfirm:            'Eliminar esta regra recorrente?',
    sharedWithParticipants:   'Partilhado com participantes',
    sharedWithParticipantsDesc: 'Os movimentos gerados são divididos entre todos os participantes da conta',
    groupExpense:      'Despesa de grupo',
    groupExpenseDesc:  'Gera uma despesa de grupo (com divisão) a cada disparo desta regra, em vez de um movimento pessoal',
    groupNotAMember:   'Não és membro deste grupo',
    dateRuleExact:            'Dia exato',
    dateRuleFirstBusinessDay: 'Primeiro dia útil',
    adjustToBusinessDay:      'Ajustar para o próximo dia útil',
    adjustToBusinessDayDesc:  'Se a data cair num fim de semana, passa para o próximo dia útil',
    ruleName:            'Nome da Regra',
    ruleNamePlaceholder: 'ex: Renda Mensal',
    ruleNamePlaceholderGroup: 'ex: Renda Mensal Partilhada',
    from:                'De',
    to:                  'Para',
    source:              'Origem',
    destination:         'Destino',
    account:             'Conta',
    selectAccount:       'Selecionar conta',
    amount:              'Valor',
    startDate:           'Data de Início',
    nameRequired:        'O nome é obrigatório',
    amountRequired:      'Obrigatório',
    amountMustBePositive: 'Tem de ser > 0',
    saveChanges:         'Guardar Alterações',
    frequencies: {
      weekly:  'Semanal',
      monthly: 'Mensal',
      yearly:  'Anual',
    },
  },
}
